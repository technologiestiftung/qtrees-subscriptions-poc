...
import { treeCollectorQueue } from "../queues.js";
...
WHERE
  sub.cron = ${cron}::cron`;
    ...
    for (const [email, subscriptions] of Object.entries(groupedSubscriptions)) {
      ...
      await treeCollectorQueue.add(`collector-${id}`, { email, subscriptions });
    }
    const count = await treeCollectorQueue.getJobCounts();
